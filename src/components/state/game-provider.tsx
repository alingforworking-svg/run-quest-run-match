...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveOnboarding = (profile: Partial<GameState["profile"]>) => { setState(s => ({ ...s, profile: { ...s.profile, ...profile } }));const client=createClient();if(client)void client.auth.getUser().then(({data})=>data.user?client.from("profiles").update({...(profile.displayName?{display_name:profile.displayName}:{}),...(profile.username?{username:profile.username.toLowerCase().replace(/[^a-z0-9_.]/g,"").slice(0,24)}:{}),...(profile.avatar?{avatar_url:profile.avatar}:{}),...(profile.preferences?{profile_preferences:profile.preferences}:{}),...(profile.preferences?.gender?{gender:profile.preferences.gender}:{})}).eq("id",data.user.id):null).then(result=>{if(result&&"error" in result&&result.error)notify(`Profile sync failed: ${result.error.message}`)});notify("Player profile saved"); };
